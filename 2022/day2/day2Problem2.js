...
// New rule: anytime it's X, we need to lose, anytime it's Y we need to draw, and anytime it's Z we need to win. So change our play to match this additional rule. 
/*
...
function tabulate(arr) {
    let score = 0
    // day 2 fun => if we encounter an X, play to lose, Y play to draw, Z play to win.
    // our "code" : Y = 2, X = 1, Z = 3, win = 6, lose = 0, draw = 3,
// win or lose: A Y = win, A X = draw, A Z lose, B Y = draw, B X = lose, B Z = win, C Y = lose, C X = win, C Z = draw
    for (let i = 0; i < arr.length; i++) {
        let temp = arr[i]
        switch (temp) {
            case 'A Y' :  // now we need to draw, so change to X
                score += 3 + 1    ///6 + 2
                break
            case 'A X' :
                score += 0 + 3   //3 + 1
